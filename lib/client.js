 ...
 * 2. mux 打开时回放的 approval/question requested（刷新恢复）不响；同 rpcId 只响一次。
 * 3. 六类事件完全独立：各自配置声音（内置合成音 / 静音 / 本地文件）与音量（0–100%）；
 * 4. 播放内置合成铃声（Web Audio）或本地音频文件（IndexedDB 存储）；
 * 5. 在设置面板注册「声音通知」区块。
 *
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function sanitize(value) {
      ...
      if (value && typeof value === 'object') {
        ...
        if (value.localFiles && typeof value.localFiles === 'object' && !Array.isArray(value.localFiles)) {
          for (f = 0; f < EVENT_KINDS.length; f++) {
            var ek = EVENT_KINDS[f]
    ...
    function soundFor(cfg, kind) {
      var field = SOUND_FIELD[kind]
      var value = field && cfg ? cfg[field] : undefined
      if (!value) return SOUND_DEFAULT[kind] || 'ding'
      return value
    }

    function volFor(cfg, kind) {
      var field = VOL_FIELD[kind]
      var v = field && cfg ? cfg[field] : undefined
      return typeof v === 'number' ? v : 1
    }

    function playFor(cfg, kind, sourceKey) {
      if (!cfg || cfg.enabled === false) return
      var sound = soundFor(cfg, kind)
      if (!sound || sound === 'none' || sound === 'local') return
      if (crossTab && sourceKey) {
        playWithCrossTabDedupe(sound, sourceKey, cfg, volFor(cfg, kind))
        return
      }
      playSound(sound, sourceKey, cfg, volFor(cfg, kind))
    }
    ...
    function playJobTerminal(cfg, sid, job, prev, skipCurrent) {
      var terminal = job.status === 'completed' || job.status === 'failed' || job.status === 'killed'
      if (!((prev === 'running' || prev === 'stopping') && terminal)) return
      if (job.status === 'failed') playFor(cfg, 'failure', 'job:' + sid + ':' + job.id)
      else if (job.status === 'killed') return
      else if (!skipCurrent) playFor(cfg, 'completion', 'job:' + sid + ':' + job.id)
    }
    ...
    function startMuxWatcher(ctx, store, onGiveUp) {
      ...
      function handleJobs(cfg, sid, jobs) {
        ...
        for (var i = 0; i < jobs.length; i++) {
          ...
          playJobTerminal(cfg, sid, job, prev, skipCurrentSession(cfg, sid))
      ...
      function handleFrame(envelope) {
        ...
        if (frame.type === 'approval/requested' || frame.type === 'question/requested') {
          ...
          if (frame.type === 'approval/requested') {
            playFor(cfg, 'approval', 'pending:' + sid + ':approval')
            return
          }
          var kind = resolveQuestionKind(ctx, sid, frame.questions)
          playFor(cfg, kind, 'pending:' + sid + ':' + kind)
          return
        }
        ...
        if (frame.type === 'session/event') {
          ...
          if (endKind === 'completed' && !skipCurrentSession(cfg, sid)) {
            playFor(cfg, 'completion', 'session:' + sid)
          } else if (endKind === 'error') {
            playFor(cfg, 'failure', 'session:' + sid + ':failure')
          }
          ...
          if (frame.key === 'goal' && frame.value && typeof frame.value === 'object' && typeof frame.value.phase === 'string') {
            ...
            if (phase === 'blocked' && prev !== 'blocked') {
              playFor(cfg, 'goal-blocked', 'goal:' + sid)
            }
    ...
    function startHostWatcher(ctx, store) {
      ...
      function handleEnvelope(envelope) {
        ...
        if (frame.type === 'host/agent-error' && frame.sessionId) {
          playFor(cfg, 'failure', 'session:' + frame.sessionId + ':failure')
        }
      }
      ...
      function check() {
        ...
        for (var i = 0; i < ids.length; i++) {
          ...
          if (was === true && row.running === false && !(quietCurrent && snap.current === id)) {
            playFor(cfg, 'completion', 'session:' + id)
          }
          ...
          if (pi !== undefined && pi !== prevPi) {
            ...
            playFor(cfg, kind, 'pending:' + id + ':' + pi)
          }
          ...
          if (phase === 'blocked' && prevPhase !== 'blocked') {
            playFor(cfg, 'goal-blocked', 'goal:' + id)
          }
    ...
    var EVENTS = [
      { kind: 'completion', label: '完成' },
      { kind: 'approval', label: '审批' },
      { kind: 'question', label: '提问' },
      { kind: 'plan-review', label: '计划评审' },
      { kind: 'goal-blocked', label: '目标受阻' },
      { kind: 'failure', label: '失败' },
    ]
    ...
    function LocalFileRow(props) {
      ...
      var label = props.isData ? '内嵌音频' : (name || (props.audioId ? '本地文件' : '尚未选择文件'))
    ...
    function EventRow(props) {
      ...
      var soundKey = soundKeyOf(sound, kind)
      ...
      var playKey = function (key) {
        if (key === 'none') return
        if (key === 'local') {
          var stashed = props.cfg.localFiles && props.cfg.localFiles[kind]
      ...
      function Section() {
        ...
        if (!cfg) {
          return React.createElement('div', { className: 'dns-notify-page' },
            React.createElement('div', { className: 'dns-notify-hint' }, '声音通知配置暂不可用。'))
        }
        ...
        var setSound = function (kind, value) {
          var field = SOUND_FIELD[kind]
          var cur = cfg[field]
          var files = Object.assign({}, cfg.localFiles || {})
          var patch = {}
          if (value === 'local') {
            var stashed = files[kind]
            patch[field] = isFileSound(stashed) ? stashed : (isFileSound(cur) ? cur : 'local')
            store.patch(patch)
            return
          }
          if (isFileSound(cur)) files[kind] = cur
          patch[field] = value
          patch.localFiles = files
          store.patch(patch)
        }
        var setVolume = function (kind, value) { setField(VOL_FIELD[kind], value) }
        var onPicked = function (kind, file) {
          storeUploaded(file).then(function (value) {
            if (!value) return
            var field = SOUND_FIELD[kind]
            var c0 = configOf(store.getSnapshot())
            var prev = c0 && c0[field]
            var stashed = c0 && c0.localFiles && c0.localFiles[kind]
            ...
            if (c) playImmediate(value, c, volFor(c, kind))
          })
        }
        ...
        var eventRows = EVENTS.map(function (e) {
          return React.createElement(EventRow, {
            ...
            onSound: function (v) { setSound(e.kind, v) },
            onVolume: function (v) { setVolume(e.kind, v) },
            onPicked: function (file) { onPicked(e.kind, file) },
          })
        })
        ...
        return React.createElement('div', { className: 'dns-notify-page' },
          React.createElement('div', { className: 'dns-notify-row' },
            React.createElement('span', { className: 'dns-notify-title' }, '声音通知'),
          ...
          React.createElement('div', { className: 'dns-notify-row' },
            React.createElement('span', { className: 'dns-notify-title' }, '导入 / 导出'),
            React.createElement('div', { className: 'dns-notify-controls' },
              React.createElement('button', {
                type: 'button',
    ...
    function apply(ctx) {
      ...
      var Section = makeSection(store)
      ctx.slots.inject('settings.section', function () {
        return ctx.slots.register({
          name: 'settings.section',
          id: 'dsh-sound',
          order: 90,
          label: function () { return '声音通知' },
        }, Section)
      })
    }
